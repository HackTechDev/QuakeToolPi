...
==================
*/
surface_t *ChoosePlaneFromList
   (surface_t *surfaces, vec3_t mins,
    vec3_t maxs, qboolean usefloors, qboolean usedetail)
{
	int			j,k,l;
...
==================
*/
surface_t *SelectPartition (surface_t *surfaces,int *detail)
{
	int			i,j;
	vec3_t		mins, maxs;
	surface_t	*p, *bestsurface;
...
//
	leaffaces += count;
	leafnode->markfaces = Q_malloc(sizeof(face_t *)*(count+1));
	i = 0;
...
void PartitionSurfaces (surface_t *surfaces, node_t *node)
{
	...
	if (!split)
	{	// this is a leaf node
      node->detail=0;   // clear just in case
...
//   if (node->detail)
/*      printf("%*c%sdetail split (%g %g %g) %g %i\n",lev,' ',node->detail?"":"non-",
         planes[split->planenum].normal[0],
         planes[split->planenum].normal[1],
         planes[split->planenum].normal[2],
         planes[split->planenum].dist,
         node->contents);*/
...
//
	...
	{
		next = p->next;
//      printf("[%i,%i]",p->has_struct,p->has_detail);
		DividePlane (p, splitplane, &frontfrag, &backfrag);

/*      printf(" split to ");
      if (frontfrag)
         printf("[%i,%i] ",
            frontfrag->has_struct,frontfrag->has_detail);
      else
         printf("NULL ");
      printf("and ");
      if (backfrag)
         printf("[%i,%i]\n",
            backfrag->has_struct,backfrag->has_detail);
      else
         printf("NULL\n");*/
