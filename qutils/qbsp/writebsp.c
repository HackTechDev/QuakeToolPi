...
#ifdef DEBUG
static int face_ref_freq[MAX_MAP_MARKSURFACES];
#endif
...
//
	...
	// emit a marksurface
      ...
      if (f->texturenum<0) continue;
		do
		{
         if (f->outputnumber<0) Error("f->outputnumber<0");
#ifdef DEBUG
         face_ref_freq[f->outputnumber]++;
#endif
...
// FIXME: are all the children decendant of padded nodes?
...
#ifdef DEBUG
   {
      ...
      {
         if (face_ref_freq[i]!=1)
            printf("face_ref_freq[%i]=%i\n",i,face_ref_freq[i]);
      }
   }
#endif
}
...
void	TEX_InitFromWad (char *path)
{
	...
	lumpinfo = Q_malloc(wadinfo.numlumps*sizeof(lumpinfo_t));
