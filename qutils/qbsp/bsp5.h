...
typedef struct visfacet_s
{
	...
	int				outputnumber;	// only valid for original faces after
										   // write surfaces
   qboolean       detail;        // Is a detail face - Alexander Malmberg
...
typedef struct surface_s
{
   ...
   qboolean    has_detail;       // 1 if the surface has detail brushes
   qboolean    has_struct;       // 1 if the surface has non-detail brushes
